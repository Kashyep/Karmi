  ...
  <div class="app-shell">
    ...
    <main id="main-content" tabindex="-1">
      ...
      <section id="usage" class="section" aria-labelledby="usage-heading">
        ...
        <div class="usage-card"><div><h3>Current plan</h3><p class="value-unavailable">Unavailable until signed in.</p></div>
          <div><h3>Capacity</h3><p class="value-unavailable">No balance shown without server data.</p></div>
          <div><h3>Reset</h3><p class="value-unavailable">Reset time unavailable.</p></div></div>
    ...
    <footer class="footer"><span>Daily assistant</span><span>Server status: unavailable</span></footer>
  </div>
...
_SCRIPT = r'''(() => { const toggle = document.querySelector('.menu-toggle'); const nav = document.querySelector('.primary-nav'); if (toggle && nav) { toggle.addEventListener('click', () => { const open = nav.classList.toggle('is-open'); toggle.setAttribute('aria-expanded', String(open)); }); } const form = document.querySelector('.composer'); if (form) form.addEventListener('submit', (event) => event.preventDefault()); })();'''
